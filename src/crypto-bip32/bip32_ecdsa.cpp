...
int hdnode_from_xpub(uint32_t depth, uint32_t child_num,
    ...
    // When importing a serialized extended public key,
    // implementations must verify whether the X coordinate in the public key data corresponds to a point on the curve.If not, the extended public key is invalid.
    CurvePoint p;
